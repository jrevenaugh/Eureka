...
server <- function(input, output, session) {
  ...
  output$airyplot <- renderPlot({
    ...
    if (Hc + Hroot + input$AHelev < 0) {
      ...
      if (input$Alandorsea == 2) {
        ...
        g <- ggplot() +
          geom_rect(data = w.df, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                    fill = "lightsteelblue" ) +
      ...
      g <- g +
        ...
        geom_rect(data = m.df,
                  ...
                  fill = "aquamarine4") +
        geom_rect(data = loc.df,
                  aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                  fill = "darkkhaki") +
        geom_rect(data = roc.df,
                  aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                  fill = "darkkhaki") +
        geom_rect(data = ec.df,
                  aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                  fill = "darkkhaki") +
  ...
  observeEvent(input$airyhelp, {
    showModal(modalDialog(
      title = "Airy Isotasy",
      HTML(paste("Airy's model of isostasy assumes uniform density in the crust.",
                 "To balance mass excesses (mountains)",
                 "or mass deficits (valleys), crustal thickness is varied,",
                 "resulting in deep crustal roots under mountains",
  ...
  observeEvent(input$pratthelp, {
    showModal(modalDialog(
      title = "Pratt Isotasy",
      HTML(paste("Pratts's model of isostasy assumes uniform crustal thickness. ",
                 "To balance mass excesses (mountains) ",
                 "or mass deficits (valleys), crustal density is varied,",
                 "resulting in low density crust under mountains ",
                 "and high density crust beneath valleys.",
                 tags$br(), tags$br(),
                 "See how density varies for mountains and valleys, both on land and on sea. ",
                 "Vary crustal thickness and nominal density to determine the",
                 "sensitivity of Pratts's model." ) ),
      easyClose = TRUE)
    )
  })
  ...
  output$freeboardplot <- renderPlot({
    ...
    # Build plot
    g <- ggplot( ) +
      geom_rect(data = cc.df,
                aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                fill = "darkkhaki") +
      geom_rect(data = oc.df,
                aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                fill = "grey60") +
      geom_rect(data = w.df,
                aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
      ...
      annotate("text", x = 2, y = cc.df$ymin + 1,
               label = "Continental Crust", size = 6, hjust = 0, vjust = 0) +
  ...
  observeEvent(input$freehelp, {
    showModal(modalDialog(
      ...
      HTML(paste("Continental freeboard is the average elevation of the continents above sea level.  ",
                 ...
                 tags$br(), tags$br(),
                 "At present, freeboard is about 850 m. ",
                 "To get there in this exercise, adjust one parameter at a time ",
                 "to see how it affects the result. ",
                 "Once you understand that, work on getting a successful ",
